...
| `CONSOLE_LOG.txt` | `bin/lib/provenance.sh` (`start_console_log`) | from job start | full stdout+stderr transcript, chronologically interleaved — all 13 pipelines (v7.3.0+) |
| `software_versions.txt` | `bin/lib/provenance.sh` (`capture_software_versions`) | pre-run, once container/tool is confirmed | real version strings queried live from the container/tool — coverage depth varies by pipeline architecture (single-container, multi-container, native, Nextflow-managed-external, orchestrator); see `CLAUDE.md` §"Provenance README" — all 13 pipelines (v7.3.0+) |
| `PROVENANCE_README.md` | `bin/lib/provenance.sh` (`generate_provenance_readme`) | job exit (success or failure) | single Hyperion-branded report: status, parameters, software versions, exact commands, and a signpost to every other artifact above — all 13 pipelines (v7.3.0+) |

For SQANTI3 (which submits 4 separate stage jobs rather than running the pipeline itself), `invocation.log` in the orchestrator's run dir captures all 4 `sbatch` calls; per-stage node capture, console log, software versions, and provenance READMEs are not available (those stage scripts live in the `containers/sqanti3` submodule) — the orchestrator's own `PROVENANCE_README.md` covers config generation and the 4 `sbatch` dispatches only.
