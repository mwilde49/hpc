...
|----------|------|---------|
| AddOne | Inline container | Demo/template pipeline |
| BulkRNASeq | Submoduled container | Bulk RNA-seq (STAR aligner) |
| Psoma | Submoduled container | Psomagen RNA-seq (HISAT2 + Trimmomatic) |
...
## Current Framework State (v7.3.2)

### What's new since v2.0

| Version | What was added |
|---------|---------------|
| v3.0.0 | Psoma pipeline integration |
| v3.1.0 | Stage-out archiving (scratch → work) |
| v4.0.0 | Hyperion Compute branding, validation fixes |
| v5.0.0 | 10x Genomics pipelines (Cell Ranger, Space Ranger, Xenium Ranger) |
...
| v7.3.2 | `bin/tjp-test-local` (Juno-free smoke testing) + tier-0 synthetic reference fixtures; fixed a real regression where psoma/bulkrnaseq silently never wrote `software_versions.txt` (argument-order bug, found by actually running the pipelines locally) |

### Titan Integration Roadmap

Local PLR-xxxx metadata records are generated by every `tjp-launch`. When Titan
comes online (~Q4 2026), `labdata` will switch to writing to the Titan PostgreSQL database
with no user-visible interface change.

Current: `labdata find runs` reads local JSON files at `/work/$USER/pipelines/metadata/pipeline_runs/`
Future: same `labdata` commands → Titan DB

See `metadata/SCHEMA.md` for full JSON schema and Titan field reference.
