...
- **Psoma and BulkRNASeq runs also get a `PROVENANCE_README.md`** (v7.2.0+ — more pipelines coming) — open this file first. It's a single, polished, self-contained report that pulls together everything else in the run directory:
  ...
  - A **software versions table** — real version strings (e.g. `HISAT2 2.2.1`, `Trimmomatic 0.39`) queried live from the run's own container, not just a checksum, because these containers don't pin tool versions at build time
  - The **exact pipeline invocation** from `invocation.log`, plus one representative resolved shell command per pipeline step (e.g. the actual `hisat2 ... | samtools sort ...` that ran), so you can see precisely what tools were called and how
  ...
  It's generated automatically at the end of every run — including failed ones, so you always have something to hand a labmate (or attach to a bug report) that explains exactly what ran and why it succeeded or failed.
