...
WRAPPER=$PROJECT_ROOT/containers/10x/bin/spaceranger-run.sh
...
trap '_EC=$?; finalize_juno_env "$RUN_DIR" "$_EC"; generate_provenance_readme "$RUN_DIR" "spaceranger" "Space Ranger — 10x Genomics Visium Spatial Transcriptomics" "$_EC" "native:spaceranger" ""' EXIT

# --- Pre-flight checks ---

if [ -z "$CONFIG" ]; then
    echo "Usage: sbatch spaceranger_slurm_template.sh <config.yaml> <run_dir> <scratch_output_dir> <fastq_dir>"
    exit 1
fi
...
if [ ! -f "$WRAPPER" ]; then
    echo "ERROR: Space Ranger wrapper not found: $WRAPPER"
    echo "Run: git submodule update --init --recursive"
    exit 1
fi

# --- Software version capture ---
capture_software_versions "$RUN_DIR" "spaceranger" "$CONFIG" "$TENX_REPO_ROOT"
