...
trap '_EC=$?; finalize_juno_env "$RUN_DIR" "$_EC"; generate_provenance_readme "$RUN_DIR" "dpnvisium-gpu" "dpnvisium — Cell2Location Visium Deconvolution (GPU: H100)" "$_EC" "$CONTAINER" ""' EXIT

# --- Pre-flight checks ---

if [ -z "$CONFIG" ]; then
    echo "Usage: sbatch dpnvisium_gpu_slurm_template.sh <config.yaml> [run_dir] [scratch_output_dir]"
    exit 1
fi
...
capture_software_versions "$RUN_DIR" "dpnvisium-gpu" "$CONTAINER"

# --- Run pipeline ---

echo "====================================================================="
echo "  HYPERION COMPUTE — dpnvisium Pipeline Executing (GPU: H100)"
echo "====================================================================="
...
echo "this is why H100 (80GB VRAM), not A30 (24GB), is the target partition."
echo "See DPNVISIUM_HPC_GUIDE.md."
