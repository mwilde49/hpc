...
Container repo `mwilde49/virome-pipeline` is a git submodule at `containers/virome/`, pinned to `v2.11.0`. This same submodule also holds the `blast_verify.nf`/`pathseq_verify.nf` offshoots — neither is wired into `tjp-launch`/`tjp-batch`, so they must be run manually from `containers/virome/` (e.g. `nextflow run blast_verify.nf -profile slurm -params-file assets/config_blast_pd19.yaml`) — and the `telescope_verify.nf` offshoot, which *is* registered as its own pipeline (`virome-telescope`, see below).
...
- **Submodule pinned to `v2.11.0`** (`cda8e7c`), a tagged release — validated end-to-end on real data (Iadorola TG cohort, TG12 sample, HSV-1-positive, ~33M real read pairs), not just the synthetic smoke test. Getting there required two real fixes past the commit (`d005289`) that first added `telescope_verify.nf`: a `conf/base.config` resource-allocation gap (`STAR_REALIGN_MULTIMAP`/`TELESCOPE_ASSIGN` had no `withName` override and would OOM loading the 33 GB GRCh38 index — fixed in `1e8553d`, which also added `scripts/run_telescope_config.sbatch`, the manual-launch sibling script this whole registration was built to replace) and a genuine upstream Telescope bug (`mlbendall/telescope#9`, a `'cached'`-mate crash on multi-mapping reads, open since 2019 — fixed in `cda8e7c`). Both are included as of `v2.11.0`.
